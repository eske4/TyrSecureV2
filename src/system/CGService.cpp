...
sys::CGroup CGService::create(std::string_view name) {
  ...
  // reference
  return sys::CGroup(std::string(name), target_path, std::move(cg_fd), cg_id);
}

bool CGService::writeCG(const sys::CGroup &cgroup, const std::string &file_name,
...
bool CGService::setMemoryLimit(const sys::CGroup &cgroup, size_t max_bytes) {
  return writeCG(cgroup, "memory.max", std::to_string(max_bytes));
}

bool CGService::setProcLimit(const sys::CGroup &cgroup, int max_pids) {
  return writeCG(cgroup, "pids.max", std::to_string(max_pids));
}

bool CGService::setCpuLimit(const sys::CGroup &cgroup,
                            std::string_view weight) {
  // weight is usually 1-10000, default 100
  return writeCG(cgroup, "cpu.weight", weight);
}

bool CGService::enableSubtreeControllers(const sys::CGroup &parent_cgroup) {
...
bool CGService::killProcs(const sys::CGroup &cgroup) {
